// Autogenerated below this point - do not edit - use imazen/licensing test/support/licenses/license_strings.rs 
fn create_licenses_hash() -> HashMap<&'static str, &'static str>{
  let mut map = HashMap::new();
  map.insert("V4_ELITE_REMOTE_PLACEHOLDER", V4_ELITE_REMOTE_PLACEHOLDER);
  map.insert("V4_ELITE_REMOTE_REMOTE", V4_ELITE_REMOTE_REMOTE);
  map.insert("V4_DOMAIN_REMOTE_SUBSCRIPTION_PLACEHOLDER", V4_DOMAIN_REMOTE_SUBSCRIPTION_PLACEHOLDER);
  map.insert("V4_DOMAIN_REMOTE_SUBSCRIPTION_REMOTE", V4_DOMAIN_REMOTE_SUBSCRIPTION_REMOTE);
  map.insert("HARD_REVOCATION_PLACEHOLDER", HARD_REVOCATION_PLACEHOLDER);
  map.insert("HARD_REVOCATION_REMOTE", HARD_REVOCATION_REMOTE);
  map.insert("HARD_REVOCATION_IMAGEFLOW_PLACEHOLDER", HARD_REVOCATION_IMAGEFLOW_PLACEHOLDER);
  map.insert("HARD_REVOCATION_IMAGEFLOW_REMOTE", HARD_REVOCATION_IMAGEFLOW_REMOTE);
  map.insert("SOFT_REVOCATION_PLACEHOLDER", SOFT_REVOCATION_PLACEHOLDER);
  map.insert("SOFT_REVOCATION_REMOTE", SOFT_REVOCATION_REMOTE);
  map.insert("SOFT_REVOCATION_IMAGEFLOW_PLACEHOLDER", SOFT_REVOCATION_IMAGEFLOW_PLACEHOLDER);
  map.insert("SOFT_REVOCATION_IMAGEFLOW_REMOTE", SOFT_REVOCATION_IMAGEFLOW_REMOTE);
  map.insert("PERPETUAL_PLACEHOLDER", PERPETUAL_PLACEHOLDER);
  map.insert("PERPETUAL_REMOTE", PERPETUAL_REMOTE);
  map.insert("CANCELLED_PLACEHOLDER", CANCELLED_PLACEHOLDER);
  map.insert("CANCELLED_REMOTE", CANCELLED_REMOTE);
  map.insert("CANCELLED_IMAGEFLOW_PLACEHOLDER", CANCELLED_IMAGEFLOW_PLACEHOLDER);
  map.insert("CANCELLED_IMAGEFLOW_REMOTE", CANCELLED_IMAGEFLOW_REMOTE);
  map.insert("SITE_WIDE_PLACEHOLDER", SITE_WIDE_PLACEHOLDER);
  map.insert("SITE_WIDE_REMOTE", SITE_WIDE_REMOTE);
  map.insert("SITE_WIDE_IMAGEFLOW_PLACEHOLDER", SITE_WIDE_IMAGEFLOW_PLACEHOLDER);
  map.insert("SITE_WIDE_IMAGEFLOW_REMOTE", SITE_WIDE_IMAGEFLOW_REMOTE);
  map.insert("OEM_PLACEHOLDER", OEM_PLACEHOLDER);
  map.insert("OEM_REMOTE", OEM_REMOTE);
  map.insert("CORES_PLACEHOLDER", CORES_PLACEHOLDER);
  map.insert("CORES_REMOTE", CORES_REMOTE);
  map.insert("PER_CORE_2_DOMAINS_PLACEHOLDER", PER_CORE_2_DOMAINS_PLACEHOLDER);
  map.insert("PER_CORE_2_DOMAINS_REMOTE", PER_CORE_2_DOMAINS_REMOTE);
  map.insert("PER_CORE_2_DOMAINS_IMAGEFLOW_PLACEHOLDER", PER_CORE_2_DOMAINS_IMAGEFLOW_PLACEHOLDER);
  map.insert("PER_CORE_2_DOMAINS_IMAGEFLOW_REMOTE", PER_CORE_2_DOMAINS_IMAGEFLOW_REMOTE);
  map.insert("XML_PLACEHOLDER", XML_PLACEHOLDER);
  map.insert("XML_REMOTE", XML_REMOTE);
  map
}
lazy_static!{
  pub static ref LICENSES: HashMap<&'static str, &'static str> = create_licenses_hash();
}

/// Kind: id
/// Id: 4271246357
/// Secret: test150c96fee33ba1cc659e9d6170801ed127bd114f5694e78c31483b374a036542
/// Owner: Acme Corp
/// Issued: 2017-04-21T00:00:00+00:00
/// NetworkGraceMinutes: 480
/// IsPublic: false
pub const V4_ELITE_REMOTE_PLACEHOLDER: &'static str= "License 4271246357 for Acme Corp ::";

/// Id: 4271246357
/// Owner: Acme Corp
/// Features: R4Elite R4Creative R4Performance
/// Kind: v4-elite
/// Issued: 2017-04-21T00:00:00+00:00
/// IsPublic: true
/// MustBeFetched: true
pub const V4_ELITE_REMOTE_REMOTE: &'static str= "License 4271246357 for Acme Corp :SWQ6IDQyNzEyNDYzNTcKT3duZXI6IEFjbWUgQ29ycApGZWF0dXJlczogUjRFbGl0ZSBSNENyZWF0aXZlIFI0UGVyZm9ybWFuY2UKS2luZDogdjQtZWxpdGUKSXNzdWVkOiAyMDE3LTA0LTIxVDAwOjAwOjAwKzAwOjAwCklzUHVibGljOiB0cnVlCk11c3RCZUZldGNoZWQ6IHRydWU=:";

/// Kind: id
/// Id: 3168226392
/// Secret: test584cd7bcd3980539d2ef8c43960aacf906733ce66d44896852b0f813e52e5353
/// Owner: Acme Corp
/// Issued: 2017-04-21T00:00:00+00:00
/// NetworkGraceMinutes: 480
/// IsPublic: false
pub const V4_DOMAIN_REMOTE_SUBSCRIPTION_PLACEHOLDER: &'static str= "License 3168226392 for Acme Corp ::";

/// Id: 3168226392
/// Owner: Acme Corp
/// Domains: box.com stamps.com
/// Features: R4Performance
/// Kind: v4-domain
/// Issued: 2017-04-21T00:00:00+00:00
/// Expires: 2017-06-05T00:00:00+00:00
/// IsPublic: true
/// MustBeFetched: true
pub const V4_DOMAIN_REMOTE_SUBSCRIPTION_REMOTE: &'static str= "License 3168226392 for Acme Corp ::";

/// Kind: id
/// Id: 1812344071
/// Secret: test072d066cb21a8f3a732a274af7ad525acf845f24d8bad1c6c3bc83659ea1a219
/// Owner: Acme Corp
/// Issued: 2017-04-21T00:00:00+00:00
/// NetworkGraceMinutes: 480
/// IsPublic: false
pub const HARD_REVOCATION_PLACEHOLDER: &'static str= "License 1812344071 for Acme Corp ::";

/// Id: 1812344071
/// Owner: Acme Corp
/// Features: R_Elite R_Creative R_Performance
/// Product: Enterprise-wide license
/// Kind: site-wide
/// Issued: 2017-04-21T00:00:00+00:00
/// IsPublic: true
/// Valid: false
/// Message: Please contact support; the license was shared with an unauthorized party and has been revoked.
pub const HARD_REVOCATION_REMOTE: &'static str= "License 1812344071 for Acme Corp (Enterprise-wide license) ::";

/// Kind: id
/// Id: 521983367
/// Secret: test87d51c1f7d3f4a234c07c21bae2ce70715444cdd4df00506dcf136177372b244
/// Owner: Acme Corp
/// Issued: 2017-04-21T00:00:00+00:00
/// NetworkGraceMinutes: 480
/// IsPublic: false
pub const HARD_REVOCATION_IMAGEFLOW_PLACEHOLDER: &'static str= "License 521983367 for Acme Corp :S2luZDogaWQKSWQ6IDUyMTk4MzM2NwpTZWNyZXQ6IHRlc3Q4N2Q1MWMxZjdkM2Y0YTIzNGMwN2MyMWJhZTJjZTcwNzE1NDQ0Y2RkNGRmMDA1MDZkY2YxMzYxNzczNzJiMjQ0Ck93bmVyOiBBY21lIENvcnAKSXNzdWVkOiAyMDE3LTA0LTIxVDAwOjAwOjAwKzAwOjAwCk5ldHdvcmtHcmFjZU1pbnV0ZXM6IDQ4MApJc1B1YmxpYzogZmFsc2U=:";

/// Id: 521983367
/// Owner: Acme Corp
/// Features: Imageflow
/// Product: Enterprise-wide license
/// Kind: site-wide
/// Issued: 2017-04-21T00:00:00+00:00
/// IsPublic: true
/// Valid: false
/// Message: Please contact support; the license was shared with an unauthorized party and has been revoked.
pub const HARD_REVOCATION_IMAGEFLOW_REMOTE: &'static str= "License 521983367 for Acme Corp (Enterprise-wide license) ::";

/// Kind: id
/// Id: 2037987837
/// Secret: testfd397979e7652bd334a729947c3b651d0ef079535b7100dc866085ed93c7c766
/// Owner: Acme Corp
/// Issued: 2017-04-21T00:00:00+00:00
/// NetworkGraceMinutes: 480
/// IsPublic: false
pub const SOFT_REVOCATION_PLACEHOLDER: &'static str= "License 2037987837 for Acme Corp ::";

/// Id: 2037987837
/// Owner: Acme Corp
/// Features: R_Elite R_Creative R_Performance
/// Product: OEM redistribtuion license
/// Kind: oem
/// Issued: 2017-04-21T00:00:00+00:00
/// Expires: 2017-06-05T00:00:00+00:00
/// IsPublic: true
/// MustBeFetched: true
/// SubscriptionExpirationDate: 2017-04-21T00:00:00+00:00
/// Message: This license has been compromised; please contact Vendor Gamma for an updated license
/// Restrictions: Only for use within Vendor Gamma Prduct
pub const SOFT_REVOCATION_REMOTE: &'static str= "License 2037987837 for Acme Corp (OEM redistribtuion license) ::";

/// Kind: id
/// Id: 1345679125
/// Secret: test156f35507c7eee8f2c20024cf5161db4c7e1c699a0a8734d5ff74571fdb5e6ee
/// Owner: Acme Corp
/// Issued: 2017-04-21T00:00:00+00:00
/// NetworkGraceMinutes: 480
/// IsPublic: false
pub const SOFT_REVOCATION_IMAGEFLOW_PLACEHOLDER: &'static str= "License 1345679125 for Acme Corp ::";

/// Id: 1345679125
/// Owner: Acme Corp
/// Features: Imageflow
/// Product: OEM redistribtuion license
/// Kind: oem
/// Issued: 2017-04-21T00:00:00+00:00
/// Expires: 2017-06-05T00:00:00+00:00
/// IsPublic: true
/// MustBeFetched: true
/// SubscriptionExpirationDate: 2017-04-21T00:00:00+00:00
/// Message: This license has been compromised; please contact Vendor Gamma for an updated license
/// Restrictions: Only for use within Vendor Gamma Prduct
pub const SOFT_REVOCATION_IMAGEFLOW_REMOTE: &'static str= "License 1345679125 for Acme Corp (OEM redistribtuion license) ::";

/// Kind: id
/// Id: 590782408
/// Secret: testc89f3623c7a2359a643c985bfde219b9bed09d48afc54931143331955b98315f
/// Owner: Acme Corp
/// Issued: 2017-04-21T00:00:00+00:00
/// NetworkGraceMinutes: 480
/// IsPublic: false
pub const PERPETUAL_PLACEHOLDER: &'static str= "License 590782408 for Acme Corp :S2luZDogaWQKSWQ6IDU5MDc4MjQwOApTZWNyZXQ6IHRlc3RjODlmMzYyM2M3YTIzNTlhNjQzYzk4NWJmZGUyMTliOWJlZDA5ZDQ4YWZjNTQ5MzExNDMzMzE5NTViOTgzMTVmCk93bmVyOiBBY21lIENvcnAKSXNzdWVkOiAyMDE3LTA0LTIxVDAwOjAwOjAwKzAwOjAwCk5ldHdvcmtHcmFjZU1pbnV0ZXM6IDQ4MApJc1B1YmxpYzogZmFsc2U=:";

/// Id: 590782408
/// Owner: Acme Corp
/// Features: R_Elite R_Creative R_Performance
/// Product: site-wide
/// Kind: site-wide
/// Issued: 2017-04-21T00:00:00+00:00
/// IsPublic: true
/// MustBeFetched: true
/// SubscriptionExpirationDate: 2017-04-21T00:00:00+00:00
/// Message: Your subscription has expired; please renew to access newer product releases.
pub const PERPETUAL_REMOTE: &'static str= "License 590782408 for Acme Corp (site-wide) ::";

/// Kind: id
/// Id: 1577338763
/// Secret: test8b47045eb7b8ca42aa967f33ee1d014ba89f8d1ac207426b482d34b5c0d90935
/// Owner: Acme Corp
/// Issued: 2017-04-21T00:00:00+00:00
/// NetworkGraceMinutes: 480
/// IsPublic: false
pub const CANCELLED_PLACEHOLDER: &'static str= "License 1577338763 for Acme Corp ::";

/// Id: 1577338763
/// Owner: Acme Corp
/// Features: R_Elite R_Creative R_Performance
/// Product: site-wide
/// Kind: site-wide
/// Issued: 2017-04-21T00:00:00+00:00
/// IsPublic: true
/// MustBeFetched: true
/// Valid: false
/// Message: Your subscription has lapsed; please renew to continue using product.
pub const CANCELLED_REMOTE: &'static str= "License 1577338763 for Acme Corp (site-wide) ::";

/// Kind: id
/// Id: 3546789197
/// Secret: test4db567d31f7b54612b66f8572ac9d24ead869b3dd8b555d4e506d27c498c510c
/// Owner: Acme Corp
/// Issued: 2017-04-21T00:00:00+00:00
/// NetworkGraceMinutes: 480
/// IsPublic: false
pub const CANCELLED_IMAGEFLOW_PLACEHOLDER: &'static str= "License 3546789197 for Acme Corp ::";

/// Id: 3546789197
/// Owner: Acme Corp
/// Features: Imageflow
/// Product: site-wide
/// Kind: site-wide
/// Issued: 2017-04-21T00:00:00+00:00
/// IsPublic: true
/// MustBeFetched: true
/// Valid: false
/// Message: Your subscription has lapsed; please renew to continue using product.
pub const CANCELLED_IMAGEFLOW_REMOTE: &'static str= "License 3546789197 for Acme Corp (site-wide) ::";

/// Kind: id
/// Id: 2766684890
/// Secret: testda42e8a40db14c091dea84efd572933fdfe31ba9620e5fee79edb823a448b6e8
/// Owner: Acme Corp
/// Issued: 2017-04-21T00:00:00+00:00
/// NetworkGraceMinutes: 480
/// IsPublic: false
pub const SITE_WIDE_PLACEHOLDER: &'static str= "License 2766684890 for Acme Corp ::";

/// Id: 2766684890
/// Owner: Acme Corp
/// Features: R_Elite R_Creative R_Performance
/// Product: Enterprise-wide license
/// Kind: site-wide
/// Issued: 2017-04-21T00:00:00+00:00
/// Expires: 2017-06-05T00:00:00+00:00
/// IsPublic: true
/// MustBeFetched: true
/// ManageYourSubscription: https://account.imazen.io
/// Restrictions: No resale of usage. Only for organizations with less than 500 employees.
pub const SITE_WIDE_REMOTE: &'static str= "License 2766684890 for Acme Corp (Enterprise-wide license) ::";

/// Kind: id
/// Id: 980532268
/// Secret: test2cbc713a6d9853002503d33a67324dcf49bde227d711967ec6314eb12543c6bd
/// Owner: Acme Corp
/// Issued: 2017-04-21T00:00:00+00:00
/// NetworkGraceMinutes: 480
/// IsPublic: false
pub const SITE_WIDE_IMAGEFLOW_PLACEHOLDER: &'static str= "License 980532268 for Acme Corp :S2luZDogaWQKSWQ6IDk4MDUzMjI2OApTZWNyZXQ6IHRlc3QyY2JjNzEzYTZkOTg1MzAwMjUwM2QzM2E2NzMyNGRjZjQ5YmRlMjI3ZDcxMTk2N2VjNjMxNGViMTI1NDNjNmJkCk93bmVyOiBBY21lIENvcnAKSXNzdWVkOiAyMDE3LTA0LTIxVDAwOjAwOjAwKzAwOjAwCk5ldHdvcmtHcmFjZU1pbnV0ZXM6IDQ4MApJc1B1YmxpYzogZmFsc2U=:";

/// Id: 980532268
/// Owner: Acme Corp
/// Features: Imageflow
/// Product: Enterprise-wide Imageflow license
/// Kind: site-wide
/// Issued: 2017-04-21T00:00:00+00:00
/// Expires: 2017-06-05T00:00:00+00:00
/// ImageflowExpires: 2017-06-05T00:00:00+00:00
/// IsPublic: true
/// MustBeFetched: true
/// ManageYourSubscription: https://account.imazen.io
/// Restrictions: No resale of usage. Only for organizations with less than 500 employees.
pub const SITE_WIDE_IMAGEFLOW_REMOTE: &'static str= "License 980532268 for Acme Corp (Enterprise-wide Imageflow license) ::";

/// Kind: id
/// Id: 2613256515
/// Secret: test4321c39bec40289e14ab42503a533efb43dbe0e355be787bb3c89f4519f503b7
/// Owner: Acme Corp
/// Issued: 2017-04-21T00:00:00+00:00
/// NetworkGraceMinutes: 480
/// IsPublic: false
pub const OEM_PLACEHOLDER: &'static str= "License 2613256515 for Acme Corp ::";

/// Id: 2613256515
/// Owner: Acme Corp
/// Features: R_Elite R_Creative R_Performance
/// Product: OEM redistribtuion license
/// Kind: oem
/// Issued: 2017-04-21T00:00:00+00:00
/// Expires: 2017-06-10T00:00:00+00:00
/// IsPublic: true
/// MustBeFetched: true
/// ManageYourSubscription: https://account.imazen.io
/// Restrictions: Only for use within Vendor Gamma Prduct
pub const OEM_REMOTE: &'static str= "License 2613256515 for Acme Corp (OEM redistribtuion license) ::";

/// Kind: id
/// Id: 3550635254
/// Secret: testf664a2d346d8cfef90e3f4819a5c2454f03a68993f28f2bb4773039f6f08762e
/// Owner: Acme Corp
/// Issued: 2017-04-21T00:00:00+00:00
/// NetworkGraceMinutes: 480
/// IsPublic: false
pub const CORES_PLACEHOLDER: &'static str= "License 3550635254 for Acme Corp ::";

/// Id: 3550635254
/// Owner: Acme Corp
/// Features: R_Elite R_Creative R_Performance
/// Product: Per-core license
/// Kind: per-core
/// Issued: 2017-04-21T00:00:00+00:00
/// Expires: 2017-06-10T00:00:00+00:00
/// IsPublic: true
/// MustBeFetched: true
/// MaxServers: 4
/// TotalCores: 16
/// ManageYourSubscription: https://account.imazen.io
pub const CORES_REMOTE: &'static str= "License 3550635254 for Acme Corp (Per-core license) ::";

/// Kind: id
/// Id: 3140539178
/// Secret: test2ad330bb24212a764310654319b42e2658774a87e4da58dc5eca01ac193577f1
/// Owner: Acme Corp
/// Issued: 2017-04-21T00:00:00+00:00
/// NetworkGraceMinutes: 480
/// IsPublic: false
pub const PER_CORE_2_DOMAINS_PLACEHOLDER: &'static str= "License 3140539178 for Acme Corp ::";

/// Id: 3140539178
/// Owner: Acme Corp
/// Domains: acme.com acmestaging.com
/// Features: R_Performance
/// Product: Per Server | 2 Domains | Performance
/// Kind: per-core-domain
/// Issued: 2017-04-21T00:00:00+00:00
/// Expires: 2017-06-10T00:00:00+00:00
/// IsPublic: true
/// MustBeFetched: true
/// MaxServers: 4
/// TotalCores: 16
/// ManageYourSubscription: https://account.imazen.io
pub const PER_CORE_2_DOMAINS_REMOTE: &'static str= "License 3140539178 for Acme Corp (Per Server | 2 Domains | Performance) ::";

/// Kind: id
/// Id: 2974946287
/// Secret: testef1352b184d3fe7db4ceec458d70aaa48be97fdcb625848f6338e5594cc9bd35
/// Owner: Acme Corp
/// Issued: 2017-04-21T00:00:00+00:00
/// NetworkGraceMinutes: 480
/// IsPublic: false
pub const PER_CORE_2_DOMAINS_IMAGEFLOW_PLACEHOLDER: &'static str= "License 2974946287 for Acme Corp ::";

/// Id: 2974946287
/// Owner: Acme Corp
/// Domains: acme.com acmestaging.com
/// Features: Imageflow
/// Product: Per Server | 2 Domains | Performance
/// Kind: per-core-domain
/// Issued: 2017-04-21T00:00:00+00:00
/// Expires: 2017-06-10T00:00:00+00:00
/// ImageflowExpires: 2017-06-10T00:00:00+00:00
/// IsPublic: true
/// MustBeFetched: true
/// MaxServers: 4
/// TotalCores: 16
/// ManageYourSubscription: https://account.imazen.io
pub const PER_CORE_2_DOMAINS_IMAGEFLOW_REMOTE: &'static str= "License 2974946287 for Acme Corp (Per Server | 2 Domains | Performance) ::";

/// Kind: id
/// Id: 1319146175
/// Secret: testbf92a04e60b4b2362d45490e5142e47b687c2ebb5898da6bc71b697602016e6e
/// Owner: Acme Corp &lt;&gt;
/// Issued: 2017-04-21T00:00:00+00:00
/// NetworkGraceMinutes: 480
/// IsPublic: false
pub const XML_PLACEHOLDER: &'static str= "License 1319146175 for Acme Corp &lt;&gt; ::";

/// Id: 1319146175
/// Owner: Acme Corp &lt;&gt;
/// Domains: acme.com acmestaging.com
/// Features: R_Performance
/// Product: Per Server | 2 Domains | Performance
/// Kind: per-core-domain
/// Issued: 2017-04-21T00:00:00+00:00
/// Expires: 2017-06-10T00:00:00+00:00
/// IsPublic: true
/// MustBeFetched: true
/// MaxServers: 4
/// TotalCores: 16
/// ManageYourSubscription: https://account.imazen.io
pub const XML_REMOTE: &'static str= "License 1319146175 for Acme Corp &lt;&gt; (Per Server | 2 Domains | Performance) ::";
